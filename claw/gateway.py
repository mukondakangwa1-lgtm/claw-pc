...
def pub_chat(c: Chat, x_forwarded_for: str = Header(default=""), host: str = Header(default="")):
    ...
    reply, which, img = tutor.chat(handle, msg, (c.tz or "")[:64], (c.sid or "")[:40])
    return {"reply": reply, "brain": which, "image": img, "sid": getattr(tutor, "_LAST_SID", "")}
...
# ---------- v5.6 SESSION ARCHIVE ----------
@app.get("/api/public/sessions")
def pub_sessions(handle: str = "student", x_forwarded_for: str = Header(default=""), host: str = Header(default="")):
    _rate(x_forwarded_for or host or "local", limit=60)
    from claw import tutor
    return {"sessions": tutor.sessions_list((handle or "student")[:40])}

@app.get("/api/public/session")
def pub_session(handle: str = "student", sid: str = "", x_forwarded_for: str = Header(default=""), host: str = Header(default="")):
    _rate(x_forwarded_for or host or "local", limit=120)
    from claw import tutor
    m = tutor.session_msgs((handle or "student")[:40], (sid or "")[:48])
    return {"msgs": m or []}
